                                ...
                                (err, rows) => {
                                    if (err) res.status(statusCodes.queryError).json({ error: err });
                                        ...
                                        if (rows[0]) {
                                                ...
                                                (err, rows) => {
                                                    if (err) res.status(statusCodes.queryError).json({ error: err });
                                                        ...
                                                        if (rows[0]) {
                                                            ...
                                                            if (previousElectionYearIndex < 0) res.status(statusCodes.notFound).json({ winner: electionWinner, lastElectionDifference: "No previous elections" });
                                                                ...
                                                                INNER JOIN candidates AS CA on V.candidate_id = CA.candidate_id INNER JOIN citizens AS CI on CA.citizen_ssn = CI.citizen_ssn
                                                                WHERE (election_year = ? AND election_round = ? AND election_type = ?) 
                                                                GROUP BY V.candidate_id ORDER BY votes DESC;`, [previousElection.year, round, type],
                                                                    (err, rows) => {
                                                                        if (err) res.status(statusCodes.queryError).json({ error: err });
                                                                        else {
                                                                            let changePercentage = Math.round((electionWinner.votes * 100 / rows[0].votes - 100) * 100) / 100;

                                                                            res.status(statusCodes.notFound).json({ winner: electionWinner, lastElectionDifference: changePercentage });
                                                                        }
                                                                    });
                                                            }
                                                        }
                                            ...
                                            // res.status(statusCodes.success).json({ data: rows[0] });
                                        }
