...
const addVotes = (req, res) => {
  ...
  if (
    ...
    db.query(
      `SELECT election_id FROM elections WHERE election_year = ? AND election_round = ? 
        AND election_type = ?;`,
      ...
      (err, rows) => {
        if (err) res.status(statusCodes.queryError).json({ error: err });
          ...
          if (rows[0]) {
            db.query(
              `SELECT citizen_id FROM citizens WHERE citizen_ssn = ? AND citizen_nationality = ?;`,
                    ...
                    db.query('INSERT INTO votes SET ?;', body, (err, rows) => {
                      if (err)
                        ...
                        res
                          ...
                          .json({ rowID: rows.insertId });
                    });
                  } else
                    res
                      .status(statusCodes.notFound)
                      .json({ message: 'Citizen does not exist' });
                }
              }
            );
          } else {
            res
              .status(statusCodes.notFound)
              .json({ message: 'Election does not exist' });
          }
        }
      }
    );
...
const editVote = (req, res) => {
  ...
  if (!id) {
    ...
    db.query(
      'SELECT vote_id FROM votes WHERE vote_id = ?;',
      id,
      (err, rows) => {
        if (err) res.status(statusCodes.queryError).json({ error: err });
        else {
                  ...
                  if (rows[0]) {
                    ...
                    db.query(sql, params, (err, rows) => {
                      if (err)
                        ...
                        if (rows[0]) {
                          db.query(
                            `UPDATE votes SET ? WHERE vote_id = ?;`,
                            [body, id],
                            (err, rows) => {
                              if (err)
                                res
